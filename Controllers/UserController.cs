...
namespace VestaAbner.Controllers
{
    ...
    public class UserController : Controller
    {
       ...
       // public async 
        ...
        public  User showOne([FromQuery] string UserName)
        {
            var ExistUser =  _userManager.FindByNameAsync(UserName);
            if (ExistUser == null)
            {
                return null;
            }
            ...
            {
                var BKSh = _userManager.Users.Where(x => x.Id.Equals(UserName)).Include(x => x.shelveces).Select(x => new User
                {
                    UserName = x.UserName,
                    lastName = x.lastName,
                    shelveces = x.shelveces.Select(z => new Shelvecs { Name = z.Name, Titele = z.Titele }).ToList()
                }).FirstOrDefault();
                return BKSh;
            }
        }
